        ...
        if (!doc) {
            await storeAiReview(env.DB, caseRepositoryId, trainingDay, {
                ...
                reason: docRefs.length
                    ? 'The uploaded document type isn\'t supported for AI review yet (only PDF and image files are).'
...
If there are no consistency issues, return an empty array for consistencyIssues. Be specific and cite what you actually found in the document — do not invent issues that aren't there.`;
        ...
        if (!res.ok) {
            ...
            await storeAiReview(env.DB, caseRepositoryId, trainingDay, { status: 'failed', reason: `AI service returned ${res.status}.` });
            return;
        }
        ...
        try {
            ...
            await storeAiReview(env.DB, caseRepositoryId, trainingDay, { status: 'failed', reason: 'AI response could not be parsed.' });
            return;
        }
        ...
        try { await storeAiReview(env.DB, caseRepositoryId, trainingDay, { status: 'failed', reason: 'Unexpected error running AI review.' }); } catch (e2) {}
    }
}
