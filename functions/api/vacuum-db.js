...
export async function onRequestPost({ request, env }) {
    ...
    try {
        ...
        // Confirmed dead end, not a transient failure: D1's Workers binding
        // always wraps env.DB.exec() in an implicit transaction, and SQLite's
        // VACUUM can never run inside one — every attempt fails with
        // "cannot VACUUM from within a transaction". This is a Cloudflare
        // platform restriction on the binding itself, not something any
        // code running inside a Worker can work around. The only real path
        // is running it outside the binding entirely, e.g. via `wrangler`
        // (which is exactly what the repo's "Vacuum D1 database" GitHub
        // Actions workflow does).
        await logActivity(db, session.username, session.batchId, 'vacuum-db-failed', { error: e.message });
        return json({
            success: false,
            error: 'VACUUM cannot run from inside the app — Cloudflare blocks it on the Workers D1 binding (it always runs inside a transaction, and VACUUM can never run in one).',
            hint: 'Use the "Vacuum D1 database" GitHub Actions workflow on the repo instead (Actions tab > Run workflow) — that runs outside the binding and actually works.'
        }, 500);
    }
